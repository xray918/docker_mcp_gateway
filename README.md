# Docker MCP Gateway

统一管理多个 Docker MCP 容器的网关服务。

## 功能特性

- 🐳 **Docker 容器管理**: 解析 `docker run` 命令，自动创建和管理容器
- 🔀 **智能路由**: `/mcp/{server_name}` 路径自动路由到对应容器
- 📊 **实时监控**: 容器状态、资源使用、访问统计
- 🖥️ **Web Dashboard**: 可视化管理界面
- 📝 **日志查看**: 实时查看容器日志
- ♻️ **健康检查**: 自动监控容器健康状态

## 快速开始

### 安装

```bash
cd /Users/xiexinfa/demo/taoke_docker
uv sync
```

### 启动（开发模式）

```bash
uv run docker-mcp-gateway
```

或者指定端口：

```bash
PORT=18082 uv run docker-mcp-gateway
```

### 服务器部署

使用部署脚本进行生产环境部署：

```bash
# 复制配置文件
cp .env.example .env
# 根据需要编辑 .env

# 启动服务（后台运行）
./deploy.sh start

# 查看服务状态
./deploy.sh status

# 更新代码并重启
./deploy.sh deploy

# 查看日志
./deploy.sh logs
./deploy.sh logs-follow  # 实时跟踪
```

部署脚本支持的命令：
- `start` - 启动服务
- `stop` - 停止服务
- `restart` - 重启服务
- `status` - 查看状态
- `update` - 更新代码
- `deploy` - 更新代码并重启
- `logs` - 查看日志
- `logs-follow` - 实时日志
- `health` - 健康检查
- `clean` - 清理临时文件
- `install` - 安装为 systemd 服务（Linux）

### 远程部署（从本地部署到服务器）

```bash
# 1. 配置服务器信息（首次使用）
cp .env.remote.example .env.remote
# 编辑 .env.remote，设置 REMOTE_HOST、REMOTE_PASSWORD 等

# 2. 一键部署（直接运行）
./deploy_remote.sh           # 完整部署流程

# 其他命令
./deploy_remote.sh status    # 查看远程状态
./deploy_remote.sh logs      # 查看远程日志
./deploy_remote.sh ssh       # SSH 登录服务器
./deploy_remote.sh restart   # 重启远程服务
```

### 访问

- **Dashboard**: http://localhost:18082/
- **API 文档**: http://localhost:18082/docs
- **MCP 代理**: http://localhost:18082/mcp/{container_name}

## 使用方法

### 添加容器

1. 打开 Dashboard (http://localhost:8080/)
2. 点击"添加容器"按钮
3. 粘贴 `docker run` 命令，例如：

```bash
docker run -dit \
--name taoke-mcp  \
--restart always \
-p 8081:8081 \
-e ENV_URL=https://config.sinataoke.cn/api/mcp/secret \
-e ENV_SECRET=url:mcp.sinataoke.cn \
-e ENV_OVERRIDE=false \
liuliang520500/taoke-mcp
```

4. 点击"创建容器"

### 访问 MCP 服务

创建成功后，可以通过以下地址访问 MCP 服务：

```
http://localhost:8080/mcp/taoke-mcp
```

## API 接口

| 方法 | 路径 | 描述 |
|------|------|------|
| GET | /api/status | 获取网关状态 |
| GET | /api/containers | 列出所有容器 |
| POST | /api/containers | 创建容器 |
| DELETE | /api/containers/{name} | 删除容器 |
| POST | /api/containers/{name}/start | 启动容器 |
| POST | /api/containers/{name}/stop | 停止容器 |
| GET | /api/containers/{name}/logs | 获取容器日志 |
| ANY | /mcp/{name}/* | MCP 代理 |

## 配置

### 环境变量

| 变量 | 默认值 | 描述 |
|------|--------|------|
| HOST | 0.0.0.0 | 绑定地址 |
| PORT | 18082 | 监听端口 |
| CONFIG_DIR | ./config | 配置文件目录 |
| DATA_DIR | ./data | 数据目录 |
| LOG_LEVEL | INFO | 日志级别 |

### 容器配置文件

配置文件位于 `config/containers.yaml`，可以手动编辑或通过 Web 界面管理。

## 架构

```
                    ┌──────────────────────────────────┐
                    │      Docker MCP Gateway          │
                    │      http://0.0.0.0:8080         │
                    └──────────────────────────────────┘
                                   │
          ┌────────────────────────┼────────────────────────┐
          │                        │                        │
    /mcp/taoke               /mcp/other               /mcp/xxx
          │                        │                        │
   ┌──────┴──────┐          ┌──────┴──────┐          ┌──────┴──────┐
   │  taoke-mcp  │          │  other-mcp  │          │   xxx-mcp   │
   │  container  │          │  container  │          │  container  │
   └─────────────┘          └─────────────┘          └─────────────┘
```

## License

MIT
